...
async def image_search(request: Request):
    ...
    try:
        ...
        if not data or 'image' not in data:
            ...
            return JSONResponse(content={"error": "Missing 'image' field in JSON body"}, status_code=400)

        base64_image = data['image']

        # 1. Get image embedding from base64 using UNICOM
        logger.info("Generating UNICOM image embedding from base64 data...")
        embedder = clip.ClipTextEmbedder()
        query_embeddings = embedder.get_embedding(base64_image)
        if query_embeddings is None:
            # Error logged in helper function
            return JSONResponse(content={"error": "Failed to process uploaded image using UNICOM."}, status_code=400)
        logger.info("UNICOM image embedding generated.")

        # 2. Search UNICOM ChromaDB collection
        logger.info(f"Querying ChromaDB UNICOM collection '{embedder.get_collection_name}' with image embedding...")
        ...
        if search_results and search_results.get('ids') and search_results['ids'][0] and \
        ...
        return JSONResponse(content={"error": "An internal error occurred during image search."}, status_code=500)
