...
class ImageEmbeddingIngestor:
    ...
    def get_images(self) -> list[str]:
        ...
        logger.info(
            f"Searching for images in directory: {self.img_dir} with pattern: {pattern}"
        )
        ...
        logger.info(
            f"Found {len(image_paths)} images in directory: {self.img_dir}"
        )
        return image_paths
    ...
    def get_embedding(
        ...
        match self.model_type:
            case ModelType.UNICOM:
                logger.info(
                    f"Getting UNICOM embeddings for {len(img_paths)} images."
                )
            ...
            case ModelType.CLIP:
                logger.info(
                    f"Getting CLIP embeddings for {len(img_paths)} images."
                )
    ...
    def embed_clip(self, img_paths: list[str]) -> list[ImageData]:
        ...
        with tqdm(
            total=total_images, desc="Embedding images", colour="blue"
        ) as progress_bar:
            for img_path in img_paths:
                if not self.file_exists(img_path):
                    self.handle_missing_files(
                        img_path, skipped_image_count
                    )
        ...
        with tqdm(
            ...
            for img_path in img_paths:
                if not self.file_exists(img_path):
                    ...
                    continue
                try:
                    embedding = unicom.get_embedding_from_img(
                        img_path
                    )
                    if embedding is not None:
                        self.update_embedding(
                ...
                progress_bar.update(1)
        logger.info(
            f"Processed {processed_image_count} images, skipped {skipped_image_count} images."
        )
        return embeddings
    ...
    async def filter_new_images_from_db(
        self, collection_name, all_images
    ):
        ...
        try:
            ...
            for img_path in tqdm(
                all_images, desc="Filtering new images", colour="blue"
            ):
            ...
            logger.info(
                f"Image already exists in the collection: {img_path}"
            )
            ...
            if "Collection" in str(e) and "does not exist" in str(e):
                logger.warning(
                    f"Collection '{collection_name}' does not exist in ChromaDB. All images will be considered new."
                )
                return all_images
            logger.error(
                f"Error filtering new images from ChromaDB: {e}",
                exc_info=True,
            )
            return all_images
    ...
    async def process(self):
        ...
        logger.info(
            f"Starting {collection_name} image embedding process from directory: {self.img_dir}"
        )
        ...
        image_paths = await self.filter_new_images_from_db(
            collection_name, image_paths
        )
        if not image_paths:
        ...
        if img_data:
            await upsert_to_chroma(
                collection_name=collection_name, img_data=img_data
            )
            logger.info(
