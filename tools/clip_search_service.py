...
CLIP_MODEL_NAME = (
    "openai/clip-vit-base-patch32"  # Renamed for clarity
)
UNICOM_COLLECTION_NAME = (
    "biocosmos_images_unicom"  # New collection for UNICOM
)
...
logger.info(
    f"Loading CLIP model: {CLIP_MODEL_NAME} for text processing..."
)
try:
...
logger.info(
    f"Loading UNICOM model: {UNICOM_MODEL_NAME} for image processing..."
)
try:
    # Load UNICOM model (likely defaults to CPU) and transform
    _model, unicom_transform = unicom.load(
        UNICOM_MODEL_NAME
    )  # Load without specifying device initially
    ...
    logger.info(
        f"UNICOM model and transform loaded and moved to {DEVICE} successfully"
    )
except Exception as e:
    logger.error(
        f"Error loading or moving UNICOM model: {e}", exc_info=True
    )
...
if clip_model is None and unicom_model is None:
    logger.error(
        "Both CLIP and UNICOM models failed to load. Exiting."
    )
    exit(1)
elif clip_model is None:
    logger.warning(
        "CLIP model failed to load. Text search will be unavailable."
    )
elif unicom_model is None:
    logger.warning(
        "UNICOM model failed to load. Image search will be unavailable."
    )
...
try:
    ...
    logger.info(
        f"Successfully connected to ChromaDB and got collection '{CLIP_COLLECTION_NAME}'."
    )
    logger.info(
        f"Collection '{CLIP_COLLECTION_NAME}' currently has {clip_collection.count()} items."
    )

    # Get or create the UNICOM collection
    unicom_collection = chroma_client.get_or_create_collection(
        name=UNICOM_COLLECTION_NAME
    )
    logger.info(
        f"Successfully connected to ChromaDB and got collection '{UNICOM_COLLECTION_NAME}'."
    )
    logger.info(
        f"Collection '{UNICOM_COLLECTION_NAME}' currently has {unicom_collection.count()} items."
    )


except Exception as e:
    logger.error(
        f"Error connecting to or getting collection from ChromaDB: {e}",
        exc_info=True,
    )
    exit(1)
...
def get_text_embedding_clip(text):
    ...
    return (
        text_features.cpu().numpy().tolist()
    )  # Return as list of lists for Chroma
...
def search_text():  # Renamed function slightly for clarity
    if clip_model is None:
        return jsonify(
            {"error": "Text search (CLIP) is not available."}
        ), 503
    ...
    try:
        ...
        logger.info(
            f"Querying ChromaDB CLIP collection '{CLIP_COLLECTION_NAME}'..."
        )
        ...
        if (
            ...
            for i in range(len(ids)):
                ...
                if (
                    ...
                    logger.warning(
                        f"CLIP Hit {hit_id} missing 'species_folder' or 'image_filename' in metadata: {metadata}"
                    )
        else:
            logger.info(
                "ChromaDB (CLIP) returned no results or results in unexpected format."
            )
        ...
        logger.info(
            f"Returning {len(unique_results)} unique species results from CLIP text search."
        )
...
def search_by_image():
    if unicom_model is None:  # Check if UNICOM model is available
        return jsonify(
            {"error": "Image search (UNICOM) is not available."}
        ), 503
    ...
    try:
        ...
        logger.info(
            f"Querying ChromaDB UNICOM collection '{UNICOM_COLLECTION_NAME}' with image embedding..."
        )
        search_results = (
            unicom_collection.query(  # Use unicom_collection
                query_embeddings=query_embeddings,
                n_results=TOP_K,
                include=["metadatas", "distances"],
            )
...
if __name__ == "__main__":
    logger.info(
        "Starting Flask server for CLIP (text) and UNICOM (image) search..."
    )
    app.run(
        host="0.0.0.0", port=5001, debug=False
    )  # Using same port 5001
