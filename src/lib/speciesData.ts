...
export interface SpeciesData { // Exporting interface for use elsewhere
...
const staticSpeciesData: Record<string, { commonName: string; description: string; conservationStatus: string }> = {
  'danaus_plexippus': {
    commonName: "Monarch Butterfly",
    description: "The Monarch butterfly is known for its incredible long-distance seasonal migration. Larvae feed exclusively on milkweed plants, sequestering toxins that make the adults unpalatable to predators. Its distinct orange and black wing pattern is easily recognizable.",
    conservationStatus: "Endangered", // IUCN status as of recent assessments
  },
  'vanessa_atalanta': {
    commonName: "Red Admiral",
    description: "The Red Admiral is a familiar butterfly found in temperate regions across Europe, Asia, and North America. It is characterized by its velvety black wings intersected by prominent reddish-orange bands and white spots. Adults often feed on fermenting fruit and tree sap.",
    conservationStatus: "Least Concern",
  },
  'ypthima_baldus': {
    commonName: "Common Five-ring",
    description: "The Common Five-ring is a widespread butterfly found across parts of Asia and Australia. It typically flies close to the ground in grassy areas and is characterized by several prominent eyespots ('rings') on its wings.",
    conservationStatus: "Least Concern",
  },
  'zaretis_itys': {
    commonName: "Itys Leafwing",
    description: "The Itys Leafwing is known for its remarkable camouflage, resembling a dead leaf when its wings are closed. Found in Central and South America, it inhabits forest environments. The underside wing pattern provides excellent protection from predators.",
...
function processSpeciesFolder(folderName: string, baseImageDir: string): SpeciesData | null {
  const speciesFolderPath = path.join(baseImageDir, folderName);
  let imageFiles: string[] = [];

  try {
    imageFiles = fs.readdirSync(speciesFolderPath)
  ...
  const allImageUrls = imageFiles.map(file => `/images/nymphalidae_new/${folderName}/${file}`);
  ...
  const commonName = staticData?.commonName ?? (formattedName.replace(/^[A-Z]+\s/, '').replace(/^\w/, c => c.toUpperCase()) + " Butterfly");
  const description = staticData?.description ?? `Detailed placeholder description for ${formattedName}. This section will later include information about its physical characteristics, lifecycle, unique markings, and how to distinguish it from similar species. Habitat preferences and behavioral patterns observed in the wild will also be documented here.`;
  const conservationStatus = staticData?.conservationStatus ?? ["Least Concern", "Near Threatened", "Vulnerable"][Math.floor(Math.random() * 3)];
...
export async function getGenusList(familyName: string = "Nymphalidae"): Promise<GenusSummary[]> {
  ...
  if (familyName.toLowerCase() !== 'nymphalidae') {
    console.warn(`getGenusList currently only supports Nymphalidae, requested: ${familyName}`);
    return [];
  }
  ...
  const genusList: GenusSummary[] = Array.from(generaMap.entries()).map(([name, imageUrl]) => ({
    name: name,
...
export async function getSpeciesByGenus(genusName: string): Promise<SpeciesData[]> {
  const allSpecies = await getSpeciesData(); // Get all species data
  const filteredSpecies = allSpecies.filter(
    species => species.taxonomy.genus.toLowerCase() === genusName.toLowerCase()
  );
...
export async function getSpeciesData(folderName: string): Promise<SpeciesData | null>;

// Combined function implementation
export async function getSpeciesData(folderName?: string): Promise<SpeciesData[] | SpeciesData | null> {
  const baseImageDir = path.join(process.cwd(), "public", "images", "nymphalidae_new");
  ...
  if (folderName) {
    ...
    try {
      const speciesFolders = fs.readdirSync(baseImageDir, { withFileTypes: true })
