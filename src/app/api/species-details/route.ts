...
export async function GET(request: Request) {
  ...
  if (idsToFetch.length === 0) {
    return NextResponse.json(
      { error: "Query parameter 'id' or 'ids' is required" },
      { status: 400 }
    );
  }

  console.log(
    `API: Fetching species details for IDs: ${idsToFetch.join(", ")}`
  );
  ...
  try {
    ...
    // const validSpeciesData = speciesDetails.filter(
    //   (species): species is SpeciesData => species !== null
    // );
    const validSpeciesData: SpeciesData[] = []; // Placeholder until getSpeciesData is implemented
    ...
    console.error(
      `Error fetching species details for IDs [${idsToFetch.join(", ")}]:`,
      error
    );
    const errorMessage =
      error instanceof Error ? error.message : "An unknown error occurred";
    return NextResponse.json(
      { error: `Failed to fetch species details: ${errorMessage}` },
      { status: 500 }
    );
  }
}
