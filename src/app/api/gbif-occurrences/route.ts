...
const occurrenceCache = new TtlCache<unknown[]>();
...
async function fetchOccurrences(taxonKey: number): Promise<unknown[]> {
  const cacheKey = String(taxonKey);
  const cached = occurrenceCache.get(cacheKey);
  if (cached !== undefined) return cached;

  const response = await fetch(
    `${GBIF_OCCURRENCE_URL}?taxonKey=${taxonKey}&limit=${GBIF_LIMIT}` +
      `&hasCoordinate=true&hasGeospatialIssue=false`,
    { method: "GET", headers: { Accept: "application/json" } },
  );
  if (!response.ok) {
    throw new Error(
      `GBIF occurrence search failed: ${response.status} ${response.statusText}`,
    );
  }
...
export async function GET(request: Request): Promise<NextResponse> {
  ...
  if (!species) {
    return NextResponse.json(
      { error: "Query parameter 'species' is required", status: "error", results: [] },
      { status: 400 },
    );
  }
  ...
  if (candidates.length === 0) {
    return NextResponse.json({ status: "unmatched", results: [] });
  }
  ...
  try {
    ...
    for (const name of candidates) {
      ...
      const results = await fetchOccurrences(taxonKey);
      return NextResponse.json({
        status: "ok",
        matchedName: name,
        taxonKey,
    ...
    console.error(`Error fetching GBIF occurrences for ${species}:`, error);
    const message =
