...
const CLIP_SERVICE_URL = 'http://localhost:5001/search'; // Adjust port if needed
...
export async function GET(request: Request) {
  ...
  console.log(`Forwarding semantic search query \"${query}\" to ${CLIP_SERVICE_URL}`);

  try {
    // Forward the request to the local Python service
    const clipResponse = await fetch(`${CLIP_SERVICE_URL}?q=${encodeURIComponent(query)}`, {
    ...
    if (!clipResponse.ok) {
      ...
      console.error(`Error from CLIP service (${clipResponse.status}): ${errorBody}`);
    ...
    if (!Array.isArray(results)) {
        console.error('Unexpected response format from CLIP service. Expected an array.', results);
    ...
    return NextResponse.json({ error: `Failed to contact CLIP service: ${errorMessage}` }, { status: 503 }); // 503 Service Unavailable
  }
} 