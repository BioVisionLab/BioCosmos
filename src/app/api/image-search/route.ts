...
const CLIP_IMAGE_SEARCH_URL = 'http://localhost:5001/search_by_image'; // Adjust if needed
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!body.image || typeof body.image !== 'string' || !body.image.startsWith('data:image/')) {
      return NextResponse.json({ error: 'Invalid request body: base64 image string required.' }, { status: 400 });
    }
  ...
  try {
    ...
    if (!clipResponse.ok) {
      ...
      console.error(`Error from CLIP image search service (${clipResponse.status}): ${errorBody}`);
      throw new Error(`CLIP service (image search) failed with status ${clipResponse.status}`);
    }
    ...
    if (!Array.isArray(results)) {
        console.error('Unexpected response format from CLIP image service. Expected an array.', results);
    ...
    return NextResponse.json({ error: `Failed to contact CLIP service for image search: ${errorMessage}` }, { status: 503 }); // 503 Service Unavailable
  }
} 