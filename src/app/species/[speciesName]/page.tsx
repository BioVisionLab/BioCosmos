...
function formatConservationStatus(status: string) {
  ...
  switch (status) {
    case 'Least Concern': bgColor = 'bg-green-200'; textColor = 'text-green-900'; break;
    case 'Near Threatened': bgColor = 'bg-yellow-200'; textColor = 'text-yellow-900'; break;
    case 'Vulnerable': bgColor = 'bg-red-200'; textColor = 'text-red-900'; break;
...
async function fetchGbifOccurrences(scientificName: string): Promise<Occurrence[]> {
  ...
  const gbifLimit = 200;
  const gbifApiUrl = `https://api.gbif.org/v1/occurrence/search?scientificName=${encodeURIComponent(scientificName)}&limit=${gbifLimit}&hasCoordinate=true&hasGeospatialIssue=false`;
  ...
  try {
    ...
    console.log(`Fetched ${occurrences.length} valid GBIF occurrences for ${scientificName}`);
    return occurrences;

  } catch (error) {
    console.error(`Error fetching GBIF occurrences for ${scientificName}:`, error);
...
export default async function SpeciesPage({ params }: SpeciesPageProps) {
  ...
  if (!details) {
    return (
      <section className="container mx-auto px-4 py-8">
        ...
        <Link href="/" className="text-blue-600 hover:underline mt-4 inline-block">Return to homepage</Link>
      </section>
    );
  }

  // Now we know details is not null and has the SpeciesData type
  const { name, commonName, description, taxonomy, conservationStatus, imageUrl, allImageUrls } = details;
  ...
  return (
    <section> {/* Removed container from here, relies on Layout's container */}
      ...
      <nav className="text-sm mb-4 text-gray-600 dark:text-gray-400 flex items-center gap-2">
        ...
        <Link href={`/family/${taxonomy.family}`} className="hover:underline">{taxonomy.family}</Link>
        <span>&gt;</span>
        {/* Link to the Genus page */}
        <Link href={`/genus/${taxonomy.genus}`} className="hover:underline italic">{taxonomy.genus}</Link>
        <span>&gt;</span>
        <span className="italic text-gray-800 dark:text-gray-200">{taxonomy.species}</span>
      </nav>
      ...
      <div className="mb-4">
          ...
          >
              &larr; Back to <span className="italic">{taxonomy.genus}</span> species
          </Link>
      </div>
      ...
      <div className="mb-6">
        ...
        <p className="text-xl text-gray-700 dark:text-gray-300">{commonName}</p> {/* Reverted to original gray colors */}
      </div>
        ...
        <div className="lg:col-span-1 space-y-6">
          ...
          <div>
            ...
            <ul className="space-y-1 text-sm text-gray-700 dark:text-gray-300">
              <li><span className="font-medium w-20 inline-block">Kingdom:</span> {taxonomy.kingdom}</li>
              <li><span className="font-medium w-20 inline-block">Phylum:</span> {taxonomy.phylum}</li>
              <li><span className="font-medium w-20 inline-block">Class:</span> {taxonomy.class}</li>
              <li><span className="font-medium w-20 inline-block">Order:</span> {taxonomy.order}</li>
              <li><span className="font-medium w-20 inline-block">Family:</span> {taxonomy.family}</li>
              <li><span className="font-medium w-20 inline-block">Genus:</span> <i className="italic">{taxonomy.genus}</i></li>
              <li><span className="font-medium w-20 inline-block">Species:</span> <i className="italic">{taxonomy.species}</i></li>
            </ul>
          </div>
