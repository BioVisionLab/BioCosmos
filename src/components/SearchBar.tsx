  ...
  const handleSearchSubmit = async (
    ...
    if (isSemantic) {
      try {
        ...
        if (results.length > 0) {
          router.push(
            `/search?ids=${encodeURIComponent(JSON.stringify(results))}&mode=semantic`,
          );
        } else {
    ...
    <header className="bg-white dark:bg-deep-mocha-800 shadow-md sticky top-0 z-10">
      <nav className="container mx-auto px-4 py-3 flex justify-between items-center gap-4">
        ...
        <div className="flex-grow min-w-0">
              ...
              <input
                ...
                placeholder={
                  isMounted && isSemantic
                    ? "Semantic search (e.g., orange butterfly with black lines)"
                    : "Search species name..."
                }
                value={searchTerm}
          ...
          {searchError && (
            <p className="text-xs text-burnt-peach-500 mt-1">
              Error: {searchError}
            </p>
          )}
        </div>
        ...
        <div className="flex items-center space-x-4 flex-shrink-0">
          <Link href="/visualization" legacyBehavior={false}>
            <span className="hover:text-green-700 dark:hover:text-green-400 cursor-pointer whitespace-nowrap">
              Explore
            </span>
          </Link>
          <a
            href="#"
            className="hover:text-green-700 dark:hover:text-green-400 whitespace-nowrap"
          >
            Collections
          </a>
