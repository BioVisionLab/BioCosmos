...
function FunctionBadge({
  ...
  definition: (typeof SEARCH_FUNCTIONS)[number];
}) {
  return (
    <span
...
export function SemanticFunctionBadges({ toolNames }: { toolNames: string[] }) {
  return (
    <div
      ...
      aria-label="Matching search functions"
    >
      {SEARCH_FUNCTIONS.filter((definition) =>
        toolNames.includes(definition.name),
      ).map((definition) => (
  ...
  return (
    <div className="space-y-3 text-center text-sm text-deep-mocha-700 dark:text-deep-mocha-200">
      ...
      <ul
        ...
        aria-label="Search function legend"
      >
