...
class ReferenceIndex:
    """Build and reuse a compact species-level CoL DuckDB index."""
    ...
    def _build(self, path: Path, info: ColSourceInfo) -> None:
        ...
        try:
            ...
            connection.execute(
                ...
                WITH prepared AS (
                    ...
                    SELECT *,
                        lower(coalesce(structured_genus,
                            regexp_extract(
                                regexp_replace(usage_name, '\\s*\\([^)]+\\)\\s*', ' ', 'g'),
                                '^\\s*([^ ]+)', 1))) AS genus_norm,
                ...
                SELECT *, concat(genus_norm, ' ', epithet_norm) AS canonical_key
                FROM parsed
                WHERE usage_id IS NOT NULL
                  AND usage_name IS NOT NULL
                  AND taxon_rank = 'species'
                  AND genus_norm <> '' AND epithet_norm <> ''
                """
            )
            connection.execute(
                """
                ...
                SELECT
                    ...
                    epithet_norm AS accepted_epithet,
