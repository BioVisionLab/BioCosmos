...
SELECT
    ...
    selected.candidate_method AS selected_candidate_method,
    CASE
    ...
    CASE WHEN selected.accepted_id IS NOT NULL
        THEN inputs.normalized_epithet <> selected.accepted_epithet
        ELSE false END AS epithet_changed,
    coalesce(selected.candidate_count, 0)::INTEGER AS candidate_count,
    selected.runner_up_name,
    selected.runner_up_score,
